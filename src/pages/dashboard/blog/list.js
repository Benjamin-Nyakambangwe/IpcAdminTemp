...
import { BlogTableRow } from '../../../sections/@dashboard/blog';
...
function applySortFilter({ tableData, comparator, filterName, filterStatus, filterRole }) {
  ...
  if (filterName) {
    tableData = tableData.filter((item) => item.post_title.toLowerCase().indexOf(filterName.toLowerCase()) !== -1);
  }
...
export async function getServerSideProps() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/articles/articles`);
